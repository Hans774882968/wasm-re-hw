...
use crate::aes_cbc_error::AesError;
...
mod tests {
    ...
    fn aes_cbc_enc_basic_test() -> anyhow::Result<()> {
        ...
        let b64_cipher = aes_cbc_encrypt(plain, key, iv)?;
    ...
    fn aes_cbc_enc_with_spaces_test() -> anyhow::Result<()> {
        ...
        let result1 = aes_cbc_encrypt("hans7 ", key, iv)?;
        ...
        let result4 = aes_cbc_encrypt("   有空 格    ", key, iv)?;
    ...
    fn aes_cbc_dec_basic_test() -> anyhow::Result<()> {
        ...
        let decrypted = aes_cbc_decrypt(&b64_cipher, key, iv)?;
    ...
    fn aes_cbc_dec_with_spaces_test() -> anyhow::Result<()> {
        ...
        let result1 = aes_cbc_decrypt("  UDI6Jofr+62ZEcwOukh1JQ== ", key, iv)?;
        assert_eq!(result1, "hans7");
        let result2 = aes_cbc_decrypt(" \n\tIjXkq9Oi+qmFQa8Kr63jAQ==", key, iv)?;
        assert_eq!(result2, "HANS");
        let result3 = aes_cbc_decrypt("  \r\n  kYY4TkUH8rFQJF5PL3kLlw== \t", key, iv)?;
        assert_eq!(result3, "爱拼才会赢");
        let result4 = aes_cbc_decrypt("   84At44OKgcfjEp2fhbdyWg==    ", key, iv)?;
